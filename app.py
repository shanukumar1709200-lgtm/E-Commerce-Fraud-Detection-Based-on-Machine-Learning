...
def predict():
    if request.method == 'POST':
        ...
        input_variables = pd.DataFrame(
            [[Transaction_Amount, Payment_Method, Product_Category, Quantity, Customer_Age, Device_Used,
              Account_Age_Days, Transaction_Hour, Address_Match]],
            columns=['Transaction Amount', 'Payment Method', 'Product Category', 'Quantity', 'Customer Age',
                     'Device Used', 'Account Age Days', 'Transaction Hour', 'Address Match'],
            index=['input'])
    ...
    return render_template('result.html', prediction_text=results, model=Model, Transaction_Amount=Transaction_Amount,
                           Pay=Pay, prod=prod, Quantity=Quantity, Customer_Age=Customer_Age, Devi=Devi,
                           Account_Age_Days=Account_Age_Days, Transaction_Hour=Transaction_Hour, Address=Address)
